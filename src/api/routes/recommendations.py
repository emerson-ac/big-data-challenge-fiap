...
from src.api.schemas.response import RecommendationItem, RecommendationResponse
...
def _build_response(
    ...
    """Assembles the API response, reporting the model actually served."""
    return RecommendationResponse(
        user_id=request.user_id,
        is_cold_start=is_cold_start,
        recommendations=items,
        model_type="popularity" if is_cold_start else model_type,
...
async def get_recommendations(
    ...
    return _build_response(
        request, items, is_cold_start, service.model_type, elapsed_ms
    )
